...
export async function runNpmBrowser(args: string[], cwd: string, io: NpmIO) {
  try {
    ...
    const patchCtor = (CtorName: 'Error' | 'TypeError') => {
      const Orig = (globalThis as any)[CtorName]
      if (!Orig || (Orig as any).__patched_for_npm__) return
      const Patched: any = function (this: any, ...a: any[]) {
        ...
        try {
          ...
          if (!d || d.get || d.set || d.writable === false) {
            Object.defineProperty(err, 'message', { value: String(err.message ?? ''), writable: true, configurable: true })
          }
        } catch {}
        return err
      }
      ...
      ;(globalThis as any)[CtorName] = Patched
    }
    patchCtor('Error')
    patchCtor('TypeError')
    ...
    const ensureWritableMessage = () => {
      try {
        const pd = Object.getOwnPropertyDescriptor(Error.prototype as any, 'message')
        if (!pd || pd.set) return
        Object.defineProperty(Error.prototype as any, 'message', {
          configurable: true,
          enumerable: pd.enumerable ?? false,
          get: pd.get || function (this: any) { return (this as any)._message ?? '' },
          set: function (this: any, v: any) {
            try {
              Object.defineProperty(this, 'message', { value: String(v), writable: true, configurable: true })
            } catch {}
          }
        } as any)
      } catch {}
    ...
    if (origFetch && !(origFetch as any).__wrapped_for_npm__) {
      const wrapped = (...a: any[]) => origFetch(...a).catch((err: any) => Promise.reject(new Error(String(err?.message ?? err))))
      ;(wrapped as any).__wrapped_for_npm__ = true
    ...
    const cliArgs = (() => {
      ...
      const has = (f: string) => base.some(a => a === f || a.startsWith(f + '='))
    ...
    await runNpmCli(cliArgs, {
      ...
      fs,
      cwd,
      stdout: chunk => { io.write(String(chunk).replace(/\n/g, '\r\n')) },
      stderr: chunk => { io.write(String(chunk).replace(/\n/g, '\r\n')) }
    })
  } catch (e: any) {
