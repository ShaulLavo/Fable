...
export default function ChatModelDropdown() {
	...
	const trigger = (
		<button
			class={cn(
				'text-xs px-2 py-1 border rounded hover:opacity-100 opacity-80'
			)}
		...
		<div class="min-w-[320px] max-w-[480px] text-xs">
			...
			>
				...
				<div>
					...
					<Show
						when={vercelReady()}
						fallback={<div class="opacity-75">Not configured</div>}
					>
						<Show
							when={!loading()}
							fallback={<div class="opacity-75">Loading…</div>}
						>
							<div class="space-y-1">
								<For
									each={
			...
			<div
				class="px-2 py-1 opacity-70 border-t"
				style={{ 'border-color': currentBackground() }}
			>
				Tip: set VERCEL_OIDC_TOKEN (or VITE_VERCEL_OIDC_TOKEN). You can also
				store it in localStorage/sessionStorage as VERCEL_OIDC_TOKEN for local
				dev.
			</div>
		</div>
	)
