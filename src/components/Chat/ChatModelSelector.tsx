...
export default function ChatModelSelector(props: {
	open: boolean
	onClose: () => void
}) {
	const {
	...
	return (
		<div class="absolute inset-0 z-40">
			...
			<div
				class={cn(
					'absolute left-1/2 top-20 -translate-x-1/2 w-[560px] shadow-lg rounded-md overflow-hidden'
				)}
				style={{
			...
			>
				...
				<div class="flex">
					<button
						class={cn(
							...
							activeTab() === 'local' ? 'font-semibold' : 'opacity-70'
						)}
						style={{ 'border-color': currentBackground() }}
					...
					<button
						class={cn(
							'px-4 py-2 text-sm',
							activeTab() === 'vercel' ? 'font-semibold' : 'opacity-70'
						)}
				...
				<div
					class="p-3 max-h-[420px] overflow-auto"
						...
						<Show
							...
							fallback={
								<div class="text-sm opacity-80">
									Vercel gateway not configured. Set VERCEL_OIDC_TOKEN in env.
								</div>
							}
						>
							<Show
								when={!loading()}
								fallback={<div class="text-sm opacity-80">Loading models…</div>}
							>
								<div class="space-y-2">
									<For
										each={
											vercelModels().length
												? vercelModels()
												: SUPPORTED_MODELS.map(id => ({ id, name: id }))
										}
									>
										{m => (
											<div
				...
				<div
					class="px-4 py-2 text-xs opacity-80 border-t"
					style={{ 'border-color': currentBackground() }}
				>
					Current: {provider()} · Local: {localModelId()} · Vercel:{' '}
					{vercelModelId() || DEFAULT_MODEL}
				</div>
			</div>
		</div>
	)
