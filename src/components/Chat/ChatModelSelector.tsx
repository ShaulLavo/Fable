...
import { currentBackground, dragHandleColor, secondaryBackground } from '../../stores/themeStore'
  ...
  return (
    <div class="absolute inset-0 z-40">
      ...
      <div
        ...
        style={{ 'background-color': currentBackground(), border: `1px solid ${dragHandleColor()}` }}
      >
        <div class="flex items-center justify-between px-4 py-2 border-b" style={{ 'border-color': dragHandleColor() }}>
          <div class="font-semibold">Select Model</div>
          <button class="text-sm opacity-70 hover:opacity-100" onClick={props.onClose}>Close</button>
        </div>
        <div class="flex">
          <button
        ...
        <div class="p-3 max-h-[420px] overflow-auto" style={{ 'background-color': secondaryBackground() }}>
            ...
            <div class="space-y-2">
              <For each={localModels()}>
                {(m) => (
                  ...
                  >
                    <div class="truncate pr-3" title={m.name}>{m.name}</div>
                    <button class="text-sm px-2 py-1 border rounded" style={{ 'border-color': dragHandleColor() }}>Use</button>
                  </div>
                )}
              </For>
            </div>
          ...
          <Show when={activeTab() === 'vercel'}>
            <Show when={vercelReady()} fallback={<div class="text-sm opacity-80">Vercel gateway not configured. Set VERCEL_OIDC_TOKEN in env.</div>}>
        ...
        <div class="px-4 py-2 text-xs opacity-80 border-t" style={{ 'border-color': dragHandleColor() }}>
